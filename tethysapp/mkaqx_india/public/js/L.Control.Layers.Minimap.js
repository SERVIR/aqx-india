        ...
        L.Control.Layers.Minimap = L.Control.Layers.extend({
            ...
            _createMinimap: function (mapContainer, originalLayer, isOverlay) {
                ...
                map.whenReady(function () {
                    ...
                    var lyr = originalLayer.options.name;
                    if(lyr !== undefined ){
                        var jurl = `https://worldview.earthdata.nasa.gov/config/palettes/${lyr}.json`;
                        var xhr = ajax_update_database("gen-legend",{"url":jurl});
                        xhr.done(function(result) {
                            // console.log(result);
                            minimap.removeControl(leg);
                            var cv_id = 'cv'+lyr;
                            leg = new L.Control.InfoControl({
                                position: "topright",
                                content: '<canvas id="'+cv_id+'" style="width:240px;height:2vh;"></canvas>'
                            });
                            minimap.addControl(leg);

                            var cv  = document.getElementById(cv_id),
                                ctx = cv.getContext('2d');
                            ctx.clearRect(0,0,cv.width,cv.height);
                            result['colors'].forEach(function(color,i){
                                var height = cv.height;
                                var width = cv.width;
                                var w = width / result['colors'].length;
                                ctx.beginPath();
                                ctx.fillStyle = "#"+color;
